...
class glq_rates:
    ...
    def __init__(self, mission='fism2', date='2002-01-01'):
        ...
        self.date = date
        # Observed spectrum
        spectrum = lisard_spectrum(mission=mission, date=date, sort_values='wl')
        ...
        self.log_F_polys, self.log_Phi_polys, self.rescale = (
    ...
    def smooth_spectrum(self, window=49, degree=8):
        """
        Description:
        ...
        F_wl = self.data['F_wl']
        Phi_wl = F_wl*wavelengths/const.hc
        ...
        for ibin in range(self.n_bins):
            ...
            F_Matilda[mk] = np.exp(self.log_F_polys[ibin](wl))
            F_Matilda[mk] *= self.rescale[ibin]
            Phi_Matilda[mk] = np.exp(self.log_Phi_polys[ibin](wl))
            Phi_Matilda[mk] *= self.rescale[ibin]
    ...
    def get_max_degree(self, Rn=1e-2):
        ...
        for ibin in range(self.n_bins):
            ...
            term_A = 0.0
            term_B = 0.0
            delta_nu = (self.bin_breaks[ibin+1]-self.bin_breaks[ibin])
            nu0 = (self.bin_breaks[ibin+1]+self.bin_breaks[ibin])/2
            ...
            while True:
                if i > 100:
                    ...
                    if int(max_term)**i <= math.factorial(i):
                        self.max_degree = max(self.max_degree, i)
                        break
                elif i > 300:
                    # If still a problem just use Stirling's approximation
                    # max_degree = int(max_term*np.exp(1))
                    self.max_degree = int(max_term*np.exp(1))
                    break
    ...
    def set_abscissas(self):
        if self.max_degree is None:
            ...
            return
        n_points = int((self.max_degree+1)/2+0.5)
